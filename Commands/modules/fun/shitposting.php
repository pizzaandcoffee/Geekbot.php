...
class derpingtonlove implements messageCommand{
    public static function getName() {
        return "❤";
    }

    public function getDescription() {
        return "shows how much she loves you";
    }
    ...
    public function runCommand($message) {
        $params = Utils::messageSplit($message);
        if ($params[1] == "<@!171250038465495040>"){
            if ($message->author->id == '139834955848089601') {
                $message->channel->sendMessage(":heart: :kissing_heart: ");
            } else {
                $message->reply("i already have a bf...");
            }
        } else {
            $reactions = ["that person loves you", "not a chance", "way out of your league", "maybe?", "for sure, go on a date!", "ask personally"];
            $message->channel->sendMessage($reactions[array_rand($reactions)]);
        }
        return $message;
    }
